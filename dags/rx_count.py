...
def log_success_to_db(task_id, dag_id, execution_date, success_message):
    ...
    insert_sql = """
    INSERT INTO airflow_test.air_rx_success (task_id, dag_id, execution_date, success_message)
    VALUES (%s, %s, %s, %s);
    """
...
def log_failure_to_db(task_id, dag_id, execution_date, error_message,No_of_retries):
    hook = PostgresHook(postgres_conn_id='abdm_uat_connection')
    insert_sql = """
    INSERT INTO airflow_test.air_rx_fail (task_id, dag_id, execution_date, error_message,retry_count)
    VALUES (%s, %s, %s, %s,%s);
    """
...
def export_data_staging(**kwargs):
    ...
    query = ''' 
            select  trunc(gdt_entry_date) as Date,
                    count(hrgnum_puk) as RX_count,
                    count(*) over() as source_row_count
            from hrgt_ehrjson_dtl
            where gnum_hospital_code = 93911
            and gnum_isvalid = 1
            and gnum_is_autosave = 0
    ...
    insert_sql = '''
    INSERT INTO airflow_test.air_rx_row_count (execution_date,source_row) VALUES (%s,%s);
    '''
...
def load_csv_to_postgres():
    ...
    with open('/tmp/staging_data.csv', 'r') as f:
        ...
        for row in reader:
            ...
            cursor.execute(
                'INSERT INTO airflow_test.air_rx_count (Date,RX_count) VALUES (%s, %s)',
                (row[0], row[1])
            )
...
def print_data(**kwargs):
    ...
    insert_sql = '''
                UPDATE airflow_test.air_rx_row_count
