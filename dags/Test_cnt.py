...
def log_success_to_db(task_id, dag_id, execution_date, success_message):
    ...
    insert_sql = """
    INSERT INTO airflow_test.air_test_success (task_id, dag_id, execution_date, success_message)
    VALUES (%s, %s, %s, %s);
    """
...
def log_failure_to_db(task_id, dag_id, execution_date, error_message,No_of_retries):
    hook = PostgresHook(postgres_conn_id='abdm_uat_connection')
    insert_sql = """
    INSERT INTO airflow_test.air_test_fail (task_id, dag_id, execution_date, error_message,retry_count)
    VALUES (%s, %s, %s, %s,%s);
    """
...
def export_data_staging(**kwargs):
    ...
    query = ''' 
            select  count(*) as Test_count,
		            R.gnum_test_code as Test_code,
		            T.gstr_test_name as Test_name,
                    COUNT(*) OVER() AS source_row_count
            from hivt_requisition_dtl R, hivt_test_mst T
            where R.gnum_hospital_code = 22914
            and gnum_isvalid_reqd = 1
            and T.gnum_test_code = R.gnum_test_code
            and T.gnum_isvalid = 1
            and trunc(hivt_entry_date) = trunc(sysdate)-1
    ...
    insert_sql = '''
    INSERT INTO airflow_test.air_test_row_count (execution_date,source_row) VALUES (%s,%s);
    '''
    ...
    with open('/tmp/staging_data.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['Test_count','Test_code','Test_name'])
...
def load_csv_to_postgres():
    ...
    with open('/tmp/staging_data.csv', 'r') as f:
        ...
        for row in reader:
            ...
            cursor.execute(
                'INSERT INTO airflow_test.air_test_name_count (Test_count,Test_code,Test_name) VALUES (%s, %s, %s)',
                (row[0], row[1], row[2])
            )
...
def print_data(**kwargs):
    ...
    insert_sql = '''
    UPDATE airflow_test.air_test_row_count
